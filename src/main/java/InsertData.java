...
public class InsertData {
    static void populateTable () {
        ...
        try (Connection connect = DriverManager.getConnection("jdbc:postgresql://localhost:5430/umuzi",
                "postgres", "password");
            ...
            System.out.println("ERROR: Not connected to the database, Please connect to the umuzi database");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static String sqlInsertIntoOrdersTable(int product_id, int payments_id, int fulfilledByEmployeeId, Date dateRequired, Date dateShipped, String status) {
        return "insert into customer (product_id, payments_id, fulfilled_by_employee_id, date_required, date_shipped, status)" +
                "values ('" + product_id + "','" + payments_id + "','" + fulfilledByEmployeeId + "','" +
